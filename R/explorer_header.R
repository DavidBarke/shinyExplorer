...
explorer_header <- function(
  ...
  generation_r <- shiny::reactive({
    QWUtils::get_node_distance(.explorer_rvs$current_node, .root_node_r())
  })
  ...
  output$header <- shiny::renderUI({
    ...
    ancestor_list <- purrr::map(indices, function(i) {
      node <- QWUtils::get_ancestor_node(.explorer_rvs$current_node, i)
      ...
      if (rvs$ancestor_counter < i) {
        ...
        shiny::observeEvent(input[["child_link" %_% i]], {
          .explorer_rvs$current_node <- QWUtils::get_ancestor_node(.explorer_rvs$current_node, i)
        })
